...
void DocumentHandler<T>::addDocument(shared_ptr<T> doc) {
    documents->push_back(std::move(doc));  // Correct use of push_back
}
...
void DocumentHandler<T>::digitStatistics() const {
    ...
    for (int i = 0; i < 16; ++i) {
        char hexDigit = (i < 10) ? static_cast<char>('0' + i) : static_cast<char>('A' + i - 10);
        if (digitFrequency[i] < minFreq && digitFrequency[i] > 0) {  // Ignore if frequency is 0
            minFreq = digitFrequency[i];
            minDigits.clear();
            minDigits.push_back({hexDigit, digitFrequency[i]});
        ...
        if (digitFrequency[i] > maxFreq) {
            ...
            maxDigits.clear();
            maxDigits.push_back({hexDigit, digitFrequency[i]});
        } else if (digitFrequency[i] == maxFreq) {
            maxDigits.push_back({hexDigit, digitFrequency[i]});
        }
    }

    // Display least frequent digits
    cout << "Least frequent digits with count:" << endl;
    for (const Pair<char, int>& minDigit : minDigits) {
        cout << minDigit.first << ": " << minDigit.second << endl;
    }

    // Display most frequent digits
    cout << "Most frequent digits with count:" << endl;
    for (const Pair<char, int>& maxDigit : maxDigits) {
        cout << maxDigit.first << ": " << maxDigit.second << endl;
    }
}